...
export function AddTodoForm({ tableName, onAddTodo }) {
  ...
  const handleAddTodo = (e) => {
    ...
    if (userInput === "") {
      ...
      onAddTodo(tableName, { fields: { title: todoTitle, complete: "false" } }) //passes object literal
    ...
    <div className={styles.container}>
      <form onSubmit={handleAddTodo} className={styles.form}>
        ...
        <button type="submit" className={styles.button}>
          <BsPlusSquare className={styles.icon} size="1.2rem" />
        </button>
      </form>
    </div>
  )
